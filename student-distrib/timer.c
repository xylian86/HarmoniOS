...
datetime_t cmos_datetime() {
    ...
    uint8_t century = (uint8_t) 26;         // set specific century;

    // Convert hex representation of time into decimal
    year = (year >> 4) * 10 + (year & 0x0f);
    month = (month >> 4) * 10 + (month & 0x0f);
    day = (day >> 4) * 10 + (day & 0x0f);
    hour = ((hour >> 4) * 10 + (hour & 0x0f)+8)%24;  // adjust for no overflow
...
// a common API for time read.
file_ops_table_t cmos_if = {
    .open = cmos_open,
    .close = cmos_close,
    .write = NULL,
    .read = cmos_read
 ...
 */
int32_t cmos_open(const uint8_t* filename) {
    return 0;
}
 ...
 */
int32_t cmos_read(int32_t inode, uint32_t* offset, void* buf, int32_t nbytes) {
    if(NULL == buf || NULL == offset) return FAIL;
    char tmp[] = "0000/00/00 00:00:00\n";
    if(nbytes < strlen(tmp)) return FAIL;
...
/* int32_t cmos_close(int32_t* inode_ptr)
 * input: inode - ignored
 * output: 0 (SUCCESS)

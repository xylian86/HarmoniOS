 ...
 * It goes in the GDT. 
 * Below is the bit field definition for one entry in GDT. 
 * */
...
/* TSS structure */
/* __attribute__((packed)) means canceling alignment in the struct, an option for gcc */
 ...
 * Below is the bit field definition for on entry in IDT.
 * */
...
{ 
    ...
    struct {
        ...
        uint32_t usr_or_supervisor  : 1;
        uint32_t page_write_though  : 1;
        uint32_t page_cached        : 1;
        uint32_t accessed           : 1;
        uint32_t dirty              : 1;
...
extern pde_desc_t page_directory[PAGE_ENTRY_NUM];
extern pte_desc_t page_table[PAGE_ENTRY_NUM];
extern pte_desc_t user_page_4K[PAGE_ENTRY_NUM];
